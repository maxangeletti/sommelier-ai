...
## 2. Stato del Progetto (ciclo 2026-03-20)

| Componente | Stato | Note |
|---|---|---|
| Blocco tecnico (BE+FE) | ~96% ✅ | Foundation stabile, freeze confermato |
| Progetto end-to-end | ~83% 🔄 | MVP funzionante su iPhone, deploy live |
| GT-01 → GT-26 | 20 PASS, 3 WARN, 2 FAIL | Matrice v2.0 — WARN/FAIL documentati |
| LLM Layer | File presente ⚠️ | llm_intent_parser.py **NON integrato** nel motore — solo rule-based attivo |
| Deploy backend | ✅ Live | Render.com — sommelier-ai.onrender.com |
| App iOS su iPhone | ✅ Funzionante | Build su device fisico via Xcode |
| Dataset | 100 vini ✅ | 35 regioni, 75 vitigni, 8 paesi |
...
| Claude Code | ✅ Operativo | Opus 4.6, integrato nel workflow |

## 3. Stack Tecnico

- **Frontend:** SwiftUI (iOS) — architettura chat-style
- **Backend:** FastAPI (Python) — main.py (2700+ righe)
- **Hosting backend:** Render.com (free tier) — URL: https://sommelier-ai.onrender.com
- **Dataset:** data/wines.normalized.csv — 100 vini, 27 colonne
- **LLM Runtime:** claude-haiku-4-5-20251001 (parser semantico — **non integrato nel motore**)
- **LLM Sviluppo:** claude-opus-4-6
- **Claude Code:** v2.1.80, Opus, operativo nel repo locale
...
**🔴 Integrazione LLM nel motore (LLM Step 1)**

Il file llm_intent_parser.py esiste ma NON è integrato in run_search(). Il motore usa esclusivamente parser rule-based. L'integrazione coprirebbe query "laterali" come "voglio stupire", "amici che amano la Francia", "qualcosa di estivo ma non banale". Gap documentato nei GT-24 (etna) e GT-26 (voglio stupire).

**🔴 Fix GT-24 "etna" — parse_region**

"Etna" non è riconosciuto come zona dal parser rule-based. Fix: aggiungere al vocabolario di parse_region().

**🔴 Fix GT-26 "voglio stupire" — parse_prestige_intent**

"voglio stupire" non triggerà prestige_intent. Fix immediato: aggiungere al parser rule-based. Fix strutturale: integrazione LLM.

**🔴 LLM Explain personalizzato (LLM Step 2)**

La reason deve diventare contestuale alla query dell'utente. Il backend passa i segnali del ranking all'LLM che li trasforma in testo naturale.
...
**Sessione immediata:**
- Verifica dataset 100 vini live su Render
- Re-run GT-01→GT-26 completo su dataset 100
- Fix GT-24 (etna) e GT-26 (voglio stupire)
- Rapporto di collaudo definitivo

**Sessione dedicata (Motore):**
- Integrazione LLM intent parser in run_search()
...
*SommelierAI Project Context v1.3 — Generato il 20 marzo 2026 — Aggiornare ad ogni ciclo significativo*
