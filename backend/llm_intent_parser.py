...
LLM_TIMEOUT_SEC = float(os.getenv("SOMMELIERAI_LLM_TIMEOUT_SEC", "10.0"))  # ✅ FIX: aumentato da 4s a 10s per evitare timeout prematuri
...
def parse_intent_with_llm(query: str) -> Tuple[Dict[str, Any], bool]:
    ...
    Returns:
        (intent_dict, llm_failed): intent strutturato + flag se LLM ha fallito
    """
    if not LLM_ENABLED or not LLM_API_KEY or not query.strip():
        return EMPTY_INTENT, False  # Non attivo = non fallito

    # ✅ NUOVO: Skip LLM per query semplici (singola parola che è un termine noto)
    # Questo evita timeout inutili per "barolo", "nebbiolo", "amarone", etc.
    q_normalized = query.strip().lower()
    simple_terms = [
        # Vitigni comuni
        "nebbiolo", "sangiovese", "barbera", "montepulciano", "primitivo", "aglianico",
        "nero d'avola", "negroamaro", "corvina", "cannonau", "vermentino",
        "trebbiano", "pinot grigio", "chardonnay", "sauvignon", "prosecco",
        # Denominazioni famose
        "barolo", "barbaresco", "brunello", "chianti", "amarone", "valpolicella",
        "franciacorta", "lambrusco", "soave", "gavi", "roero",
        # Regioni
        "piemonte", "toscana", "veneto", "sicilia", "puglia", "trentino",
        "borgogna", "bordeaux", "champagne", "loira", "alsazia",
        # Colori
        "rosso", "bianco", "rosato",
    ]
    
    if q_normalized in simple_terms:
        # Query semplice riconosciuta: il rule-based gestisce perfettamente
        return EMPTY_INTENT, False
    ...
    try:
        ...
        return _validate_intent(intent), False  # Success

    except Exception:
        # Fallback silenzioso: il motore rule-based gestisce tutto
        return EMPTY_INTENT, True  # LLM fallito
...
,
        r'^(nero d\'avola|negroamaro|corvina|cannonau|vermentino)
...
if __name__ == "__main__":
    ...
    for q in TEST_QUERIES:
        ...
        print(f"  → {json.dumps(active, ensure_ascii=False)}\n")
,
        r'^(trebbiano|pinot grigio|chardonnay|sauvignon|prosecco)
...
,
        # Denominazioni famose
        r'^(barolo|barbaresco|brunello|chianti|amarone|valpolicella)
...
if __name__ == "__main__":
    ...
    for q in TEST_QUERIES:
        ...
        print(f"  → {json.dumps(active, ensure_ascii=False)}\n")
,
        r'^(franciacorta|lambrusco|soave|gavi|roero)
...
,
        # Regioni
        r'^(piemonte|toscana|veneto|sicilia|puglia|trentino)
...
if __name__ == "__main__":
    ...
    for q in TEST_QUERIES:
        ...
        print(f"  → {json.dumps(active, ensure_ascii=False)}\n")
,
        r'^(borgogna|bordeaux|champagne|loira|alsazia)
