...
def _filter_by_text_contains(df: pd.DataFrame, col: str, value: Optional[str]) -> pd.DataFrame:
    if not value:
        return df
    v = _norm_lc(value)
    if not v:
        return df
    return df[df[col].astype(str).str.lower().str.contains(v, na=False)]
...
def _score_row(row: Any, price_info: Dict[str, Any], boosts: Dict[str, bool]) -> Tuple[float, float]:
...
def run_search(query: str, sort: str = "relevance", limit: int = MAX_RESULTS_DEFAULT) -> Dict[str, Any]:
    ...
    for r in rows:
        ...
        s, pdlt = _score_row(r, price_info, boosts)
        scored.append(_build_wine_card(r, rank=0, score=s, price_delta=pdlt))

    sorted_cards = _apply_sort(scored, sort)[:limit]
