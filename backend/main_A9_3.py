...
BUILD_ID = "SommelierAI v0.2 STABILE + A/B/D (CSV schema real) + cache-safe 2026-02-22-intensityfix + locmatchfix-2026-02-23 + A7-loctokens-2026-02-23 + A8-locindex-2026-02-23 + A9-1-zonematchfix-2026-02-23 + A9-2-locweight-2026-02-23 + A9-3-valuesqrt-2026-02-23"
...
def _compute_match_score(
    ...
    # A9.2: peso location dinamico — se l'utente sta chiedendo quasi solo una località,
    # diamo più importanza alla componente geografica senza cambiare il range 0..1.
    location_driven = bool(location_terms) and not (
        bool(grapes_req)
        or bool(aromas_req)
        or bool(intensity_req)
        or bool(typology_req.get("sparkling") or typology_req.get("sweetness"))
        or bool(structured_req)
    )
    loc_weight = 0.40 if location_driven else 0.20
    ...
    if location_terms:
        parts.append((loc_weight, _row_location_match(row, location_terms)))
